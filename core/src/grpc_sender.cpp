...
bool buildFromProtoFiles(const GrpcRequest& g, DescriptorContext& ctx) {
    ...
    for (const auto& f : g.protoSource.files) {
        if (ctx.importer->Import(f) == nullptr) {
            ctx.error = "không nạp được .proto: " + ctx.errCollector->errors;
            return false;
        }
    }
...
bool buildFromDescriptorSet(const GrpcRequest& g, DescriptorContext& ctx) {
    ...
    if (!fsutil::readFile(g.protoSource.descriptorSetPath, raw)) {
        ctx.error = "không đọc được descriptorSet: " + g.protoSource.descriptorSetPath;
        return false;
    }
    gp::FileDescriptorSet fds;
    if (!fds.ParseFromString(raw)) {
        ctx.error = "descriptorSet không hợp lệ (parse fail)";
        return false;
    }
    ctx.pool = std::make_unique<gp::DescriptorPool>();
    for (const auto& file : fds.file()) {
        if (ctx.pool->BuildFile(file) == nullptr) {
            ctx.error = "build FileDescriptor thất bại: " + file.name();
            return false;
        }
    }
...
void GrpcSender::send(const ResolvedRequest& req, RequestHandle handle, IUiDelegate& delegate,
    ...
    if (g.methodType != "unary") {
        delegate.onError(handle, ApiError{ErrorKind::Unsupported,
                                          "POC chỉ hỗ trợ unary; methodType '" + g.methodType +
                                              "' chưa gửi được."});
        return;
    }
    ...
    if (g.protoSource.mode == "protoFiles") {
        ...
        delegate.onError(handle,
                         ApiError{ErrorKind::Unsupported,
                                  "protoSource 'reflection' chưa hỗ trợ ở build POC này — "
                                  "hãy dùng protoFiles hoặc descriptorSet."});
        return;
    }
    if (!built) {
    ...
    if (!svc) {
        delegate.onError(handle, ApiError{ErrorKind::Parse, "không tìm thấy service: " + g.service});
        return;
    }
    const gp::MethodDescriptor* mth = svc->FindMethodByName(g.method);
    if (!mth) {
        delegate.onError(handle, ApiError{ErrorKind::Parse, "không tìm thấy method: " + g.method});
        return;
    }
    if (mth->client_streaming() || mth->server_streaming()) {
        delegate.onError(handle, ApiError{ErrorKind::Unsupported,
                                          "method này là streaming — POC chỉ chạy unary."});
        return;
    }
    ...
    if (!reqMsg->SerializeToString(&reqBytes)) {
        delegate.onError(handle, ApiError{ErrorKind::Parse, "serialize protobuf request thất bại"});
        return;
    }
    ...
    if (cancel && cancel->isCancelled()) {
        delegate.onError(handle, ApiError{ErrorKind::Cancelled, "Đã huỷ"});
        return;
    }
    if (!status.ok()) {
    ...
    if (!respMsg->ParseFromString(respBytes)) {
        delegate.onError(handle, ApiError{ErrorKind::Parse, "parse protobuf response thất bại"});
        return;
    }
    std::string jsonOut;
    {
        ...
        if (!st.ok()) {
            delegate.onError(handle, ApiError{ErrorKind::Parse,
                                              "convert response sang JSON lỗi: " +
                                                  std::string(st.message())});
            return;
        }
    }
