...
void writeFileAtomic(const std::string& path, const std::string& content) {
    ...
    tmp += ".tmp";
    {
        ...
        if (!out) throw std::runtime_error("không mở được file tạm: " + tmp.string());
        out.write(content.data(), static_cast<std::streamsize>(content.size()));
        out.flush();
        if (!out) throw std::runtime_error("ghi file tạm thất bại: " + tmp.string());
    }
    std::error_code ec;
    ...
    if (ec) {
        ...
        fs::remove(tmp);
        if (ec) throw std::runtime_error("rename atomic thất bại: " + ec.message());
    }
}
