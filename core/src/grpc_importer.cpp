...
ImportResult GrpcImporter::parse(const std::string& input) const {
    ...
    if (isGrpcurl) {
        ...
        if (positionals.size() >= 2) {
            if (!splitServiceMethod(positionals[1], g.service, g.method)) {
                res.error = "không tách được Service/Method từ: " + positionals[1];
                return res;
            }
        }
        ...
        if (slash == std::string::npos) {
            res.error = "thiếu Service/Method (định dạng host:port/pkg.Service/Method)";
            return res;
        }
        g.target = s.substr(0, slash);
        std::string sm = s.substr(slash + 1);
        if (!splitServiceMethod(sm, g.service, g.method)) {
            res.error = "không tách được Service/Method từ: " + sm;
            return res;
        }
        g.message = "{}";
    }
    ...
    if (g.target.empty() || g.service.empty() || g.method.empty()) {
        res.error = "thiếu target/service/method";
        return res;
    }
    res.ok = true;
