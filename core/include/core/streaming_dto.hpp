// core/streaming_dto.hpp — transport-neutral streaming DTOs (SPEC_grpc_streaming). The UI only ever sees
// these; senders (gRPC/SSE/WS) converge on them (INV-1) — UI must NOT branch on the transport tag.
#pragma once
...
#include "core/request_model.hpp" // ResolvedRequest embeds RequestModel
...
// ResolvedRequest: RequestModel after resolving {{var}} + applying auth (README §8.1). Shares the model
// struct for brevity — the sender only ever receives the resolved one.
