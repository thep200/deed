...
// UI only sees these structs; do NOT leak grpc++/protobuf/libcurl/nlohmann
// types out the port.
#pragma once
...
// Array (not object) to keep order, allow duplicate keys, each line has
// enabled. (README §7.4)
struct KeyValue {
  std::string key;
  std::string value;
...
// Server-Sent Events mode (SPEC_sse §1). SSE is a *consumption mode* of an HTTP
// request, not a new type. None: plain unary (gather body). Sse: always parse
// the response as text/event-stream. Auto: stream-parse only if the response
// Content-Type is text/event-stream, else unary. Default None so ordinary HTTP
...
struct HttpRequest {
  ...
  HttpStreamMode streamMode =
      HttpStreamMode::None; // SSE consumption mode (SPEC_sse §1)
};

// SSE detection (SPEC_sse §1/§4). A request streams if streamMode is Sse/Auto,
// OR it carries an enabled `Accept: text/event-stream` header (the standard SSE
// trigger) — so users can opt in with just a header.
bool httpRequestsSse(
    const HttpRequest &h); // route to the stream path (openStream)
bool httpForcesSse(
    const HttpRequest &h); // parse as SSE regardless of response Content-Type
...
struct GrpcMethodInfo {
  ...
  std::string methodType; // unary | server_streaming | client_streaming |
                          // bidi_streaming
};
...
struct GrpcRequest {
  ...
  std::string methodType =
      "unary"; // unary | server_streaming | client_streaming | bidi_streaming
...
enum class StreamPayloadKind {
  ...
  Text,
  Binary
}; // Binary -> payload is base64
enum class StreamStatus { Ok, Error, Cancelled, Timeout };

// SPEC_websocket §2.1 — duplex deltas. Defaults keep gRPC server-streaming
// (spec 1) unchanged: direction defaults Inbound, frameType defaults Message.
enum class StreamDirection { Inbound, Outbound };
enum class StreamFrameType {
  Text,
  Binary,
  Ping,
  Pong,
...
struct StreamEvent {
  std::uint64_t seq =
      0; // 0-based, monotonic within one stream (UI appends in order)
  StreamDirection direction =
      StreamDirection::Inbound; // WS: Inbound | Outbound (others: Inbound)
  StreamFrameType frameType =
      StreamFrameType::Message; // WS frame kind; gRPC: Message
...
struct StreamMeta { // emitted at onStreamOpen
  ...
  StreamTransport transport =
      StreamTransport::Grpc; // display/telemetry ONLY — UI must NOT branch on
                             // it (INV-1)
...
// or WS/SSE (subscription) ----
enum class GqlOperation {
  Query,
  Mutation,
  Subscription,
  Auto
}; // Auto: inferred from the query text
enum class GqlSubTransport { WebSocket, Sse }; // transport for subscription
enum class GqlWsProtocol {
  GraphQlTransportWs,
  SubscriptionsTransportWs /* legacy */
};

struct GraphQlRequest {
  std::string
      url; // endpoint (https://host/graphql ; wss://host/graphql for sub)
  std::string query; // the GraphQL document
  std::string variablesJson = "{}";
  std::string
      operationName; // optional (when a document holds multiple operations)
  ...
  std::string
      connectionInitPayloadJson; // auth payload for connection_init (§6)
...
// ResolvedRequest: RequestModel after resolving {{var}} + applying auth.
// (README §8.1) Shares the same struct for brevity — sender only receives the
// resolved one.
struct ResolvedRequest {
  RequestModel model;
  std::string streamId; // set by Engine::openStream so the sender can stamp
                        // StreamMeta (empty for unary)
  // Stream ceilings (§9), stamped by Engine from EngineConfig. 0 -> sender
  // default.
...
struct ApiResponse {
  ...
  std::vector<KeyValue> headers;
  std::vector<Cookie>
      cookies; // Set-Cookie of the current response (POC, no jar)
  // common
  std::string body; // HTTP body or gRPC message JSON
  long elapsedMs = 0;
  std::int64_t sizeBytes = 0;
  std::string
      resolvedRequestDump; // resolved request (Request tab for debugging)
  // --- Streaming (SPEC_grpc_streaming §8): set when body is an assembled
  // stream array ---
  bool wasStreamed = false; // true -> body is the captured [ … ] array
  bool partial =
      false; // true -> stream ended early (cancel/error) -> array incomplete
...
struct TreeNode {
  std::string
      name; // display name (request: name field; folder: directory name)
  ...
  std::string
      methodOrType; // HTTP method, or gRPC methodType, to show as a badge
...
struct AppConfig {
  ...
  // --- Response cache (USER layer — edited in Settings; clamped ≤ ENV max).
  // RESPONSE_CACHE.md §1 ---
  ...
  bool cachePersist =
      true; // keep cache across restart (off -> RAM only, no disk attached)
};
