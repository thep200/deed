// core/field_codec.hpp — round-trip JSON-thô từng field cho editor (UI spec §3).
// UI hiển thị mỗi tab (Params/Headers/Body/Auth/Metadata/Message) là một ô JSON map
// thẳng vào field. Đây là port phụ để UI không phải tự parse JSON.
#pragma once
...
bool jsonToKeyValues(const std::string&, std::vector<KeyValue>& out, std::string& err);
...
// Định dạng JSON: pretty (indent 2) hoặc compact. Không parse được -> trả nguyên văn.
...
// Decode: nếu text là string literal JSON -> trả nội dung bên trong; ngược lại giữ nguyên.
